...
- [x] 1.6 Verify UI visuals, responsiveness, font switching, and basic theme toggle functionality in browser.
...
- [x] 3.1 Create `test.html` browser-based testing harness.
- [x] 3.2 Add unit assertions in `test.html` for `lorebook.js` matching (word boundaries, case sensitivity).
- [x] 3.3 Add unit assertions in `test.html` for `storage.js` operations (CRUD, import/export, quota checks).
- [x] 3.4 Add unit assertions in `test.html` for `api.js` budgeting arithmetic (70/30 distribution under limits).
- [x] 3.5 Perform manual validation: streaming generation lock, highlight removal on keystroke, story imports.
